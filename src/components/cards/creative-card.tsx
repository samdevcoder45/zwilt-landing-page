import { iconCreativeLinks } from "../../utils/nav-links";
import HeaderCard from "./header-card";
import clsx from "clsx";

export default function CreativeCard({ text }: { text: string }) {
  return (
    <HeaderCard className="bg-white pl-[28px] pr-[26px] py-[18px] rounded-[7px]">
      ...
      <div className="grid grid-cols-2 gap-2 md:flex md:flex-row">
        {iconCreativeLinks.map((link, index) => (
          <div key={link.id} className="flex flex-col items-center">
            <div
              className={clsx(
                ...
                index === 0 ? "rounded-30" : "rounded-full"
              )}
            >
